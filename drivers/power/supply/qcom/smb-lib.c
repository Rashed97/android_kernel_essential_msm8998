...
static int set_sdp_current(struct smb_charger *chg, int icl_ua)
{
	...
	pd_icl_ua = get_client_vote_locked(chg->usb_icl_votable, PD_VOTER);
	default_icl_ua = get_client_vote_locked(chg->usb_icl_votable, DEFAULT_VOTER);
	usb_icl_ua = get_client_vote_locked(chg->usb_icl_votable, USB_PSY_VOTER);
...
static int _smblib_vbus_regulator_enable(struct smb_charger *chg)
{
	int rc;
...
static int smblib_otg_enable(struct smb_charger *chg)
{
	...
	rc = _smblib_vbus_regulator_enable(chg);
	if (rc >= 0)
		chg->otg_en = true;
...
static int _smblib_vbus_regulator_disable(struct smb_charger *chg)
{
	int rc;
...
static int smblib_otg_disable(struct smb_charger *chg)
{
	...
	rc = _smblib_vbus_regulator_disable(chg);
	if (rc >= 0)
		chg->otg_en = false;
...
static void smblib_handle_typec_removal(struct smb_charger *chg)
{
	...
	if (rc < 0)
		smblib_err(chg, "Couldn't set USBIN_ADAPTER_ALLOW_5V_OR_9V rc=%d\n",
			rc);
